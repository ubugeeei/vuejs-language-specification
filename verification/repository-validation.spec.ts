...
describe("repository validation and catalog stability", () => {
  test(
  ...
  test("repository naming validator rejects legacy local naming artifacts", () => {
    ...
    mkdirSync(join(root, "testsuites", "runtime"), { recursive: true });
    ...
    writeFileSync(
      join(root, "testsuites", "runtime", "sample.ts"),
    ...
    expect(messages[0]?.errors).toEqual(
      expect.arrayContaining([
        ...
        expect.stringContaining("Legacy local naming artifact exists: test"),
        expect.stringContaining("Legacy local naming artifact exists: runtime/testsuites"),
